...
const MyTextPage: React.FC = () => {
  ...
  const [searchParams, setSearchParams] = useState<API.TextTaskQueryRequest>({
    ...initSearchParams,
  });
          ...
          <List.Item key={item.id}>
            <Card style={{ width: '100%' }}>
              ...
              <>
                ...
                {item.status === 'running' && (
                  <>
                    <Result status="info" title="文本生成中" subTitle={item.execMessage} />
                  </>
                )}
                {item.status === 'succeed' && (
                  <>
                    <Result status="success" title="文本转换成功" subTitle={item.execMessage} />
                  </>
                )}
                {item.status === 'failed' && (
                  <>
                    <Result status="error" title="文本生成失败" subTitle={item.execMessage} />
                  </>
                )}
              </>
            </Card>
